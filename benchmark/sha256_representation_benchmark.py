...
Modes:
  collision
      Cryptanalytic-style collision/search benchmark.
      The solver searches for two distinct message blocks satisfying
      configured differential constraints and producing the same
  ...
  equiv
      ...
      SAT   = counterexample found (representation bug)
      UNSAT = representations are formally equivalent

Validation structure:
  Phase 0: Independent Python recurrence checks.
  Gate 0:  One-round symbolic equivalence check.
  Phase 1: Selected benchmark mode execution.
  Phase 2: Independent pure-Python verification of SAT collision witnesses.
  Phase 3: Structured JSON/CSV export.
"""
...
def build_sw_explicit(
    ...
    for prefix in ("m1_", "m2_"):
        for i in range(rounds + 4):
            lines.extend([
                f"(declare-const "
                f"{prefix}a_mt_{i} (_ BitVec 32))",

                f"(declare-const "
                f"{prefix}b_mt_{i} (_ BitVec 32))",
            ])
        ...
        for i in range(rounds):
            ...
            lines.append(
                ...
                f"(bvadd (S1 {prefix}b_mt_{i+3}) "
                f"(bvadd "
                f"(ch_sw {prefix}b_mt_{i+3} "
                f"{prefix}b_mt_{i+2} {prefix}b_mt_{i+1}) "
                f"(bvadd {k} {prefix}w_{i})))))"
            )
...
# ============================================================================
# Equivalence Builders (Std == SW for arbitrary symbolic input)
# ============================================================================

def emit_symbolic_iv(prefix: str = "iv_") -> str:
    return "\n".join(
        f"(declare-const {prefix}{name} (_ BitVec 32))"
        for name in "abcdefgh"
    )


def emit_std_equiv_explicit(rounds: int) -> str:
    lines = []
    ...
    for i in range(rounds):
        ...
        lines.append(
            f"(define-fun std_t1_{i} () (_ BitVec 32) "
            f"(bvadd std_h_{i} "
            f"(bvadd (S1 std_e_{i}) "
            f"(bvadd "
            f"(ch_std std_e_{i} "
        ...
        lines.append(
            f"(define-fun std_t2_{i} () (_ BitVec 32) "
            f"(bvadd "
            f"(S0 std_a_{i}) "
            f"(Maj std_a_{i} "
            f"std_b_{i} std_c_{i})))"
...
def emit_sw_equiv_explicit(rounds: int) -> str:
    lines = []

    for i in range(rounds + 4):
        lines.extend([
            f"(declare-const sw_a_{i} (_ BitVec 32))",
            f"(declare-const sw_b_{i} (_ BitVec 32))",
        ])
    ...
    for i in range(rounds):
        ...
        lines.append(
            f"(define-fun sw_t1_{i} () (_ BitVec 32) "
            f"(bvadd sw_b_{i} "
            f"(bvadd (S1 sw_b_{i+3}) "
            f"(bvadd "
            f"(ch_sw sw_b_{i+3} "
            f"sw_b_{i+2} sw_b_{i+1}) "
            f"(bvadd {k} {w})))))"
        )

        lines.append(
            f"(define-fun sw_t2_{i} () (_ BitVec 32) "
            f"(bvadd "
            f"(S0 sw_a_{i+3}) "
            f"(Maj sw_a_{i+3} "
            f"sw_a_{i+2} sw_a_{i+1})))"
        )
...
def build_equiv_explicit(rounds: int) -> str:
    """
    Build the explicit-state equivalence model.

    PREAMBLE is included exactly once here.  The two emit_* helpers
    only emit declarations/constraints for their respective state
    representations.
    """
...
def build_equiv_inline(rounds: int) -> str:
    lines = [
        PREAMBLE,
        emit_msg_schedule(rounds, "sym_"),
        emit_symbolic_iv(),
...
EQUIV_BUILDERS = {
    "Explicit": build_equiv_explicit,
    "Inline": build_equiv_inline,
}
...
def run_collision_instance(
    ...
    code = COLLISION_BUILDERS[model_name](rounds, diff)
    ...
    try:
        ...
        if status == "sat" and proc is not None:
...
# ============================================================================
# Gate 0: One-Round Equivalence Check
# ============================================================================
...
(define-fun t1_sw () (_ BitVec 32)
  (bvadd b_mt_0
    (bvadd (S1 b_mt_3)
      (bvadd (ch_sw b_mt_3 b_mt_2 b_mt_1)
        (bvadd k w)))))
...
"""
    ...
    try:
        ...
        try:
            ...
            print(
                f" [FATAL] Gate 0 timed out after "
                f"{timeout}s ({elapsed:.3f}s)"
            )
        ...
        if result != "unsat":
            print(f" [FATAL] Gate 0 failed: {result}")
...
def print_summary(
    ...
    for rounds in sorted(
        ...
        if mode == "equiv":
            if (
                "Explicit" in medians
                and "Inline" in medians
                and medians["Inline"] > 0
            ):
                scaling = (
                    medians["Explicit"]
                    / medians["Inline"]
                ...
                print(
                    ...
                    f"{scaling:.3f}x ({direction})"
                )

            print(
                "\nEquivalence status: UNSAT = formally "
                "proven equivalent across all inputs."
            )
...
def validate_rounds(rounds: List[int]) -> None:
    ...
    if invalid:
        raise ValueError(
            f"Round counts must be in [1, 64]: {invalid}"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=120,
        help="Collision timeout in seconds",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=60,
        help="Equivalence timeout in seconds",
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        metavar="INDEX:HEX",
        help="Message-word XOR constraint",
    )

    parser.add_argument(
        "--keep-smt",
        action="store_true",
        help="Retain generated SMT2 files",
    )
    ...
    try:
        ...
        validate_rounds(args.equiv_rounds)

        if (
            args.trials < 1
            or args.timeout < 1
            or args.equiv_timeout < 1
        ):
            raise ValueError(
                "--trials and timeouts must be >= 1"
            )
    ...
    # ------------------------------------------------------------------------
    # Phase 0: Pure-Python recurrence check
    # ------------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------------
    # Gate 0: One-round SMT equivalence gate
    # ------------------------------------------------------------------------
    ...
    if args.mode == "collision":
        print("=" * 88)
        print("PHASE 1: Collision Search Benchmark")
        ...
        models = list(COLLISION_BUILDERS)
            ...
            for trial in range(args.trials):
                ...
                for model in order:
                    ...
                    print(
                        ...
                        f"{model:<14} "
                        f"{res.status:<8} "
                        f"{res.runtime_seconds:8.3f}s"
                        f"{v_str}"
                    )

                    if (
                        res.status == "sat"
                        and res.verified is not True
                    ):
                        print(
        ...
        print("=" * 88)
        print(
            "PHASE 1: Symbolic Representation "
            "Equivalence Benchmark (Std == SW)"
        )

        print(
            f"Solver : {args.solver} "
            f"({solver_version}) | "
            f"Rounds : {args.equiv_rounds} | "
            f"Trials : {args.trials}"
        )

        print("=" * 88)

        models = list(EQUIV_BUILDERS)

        for rounds in args.equiv_rounds:
            print(f"\n--- {rounds} rounds ---")
